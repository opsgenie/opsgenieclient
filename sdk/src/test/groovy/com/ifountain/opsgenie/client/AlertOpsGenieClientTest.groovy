...
class AlertOpsGenieClientTest extends OpsGenieClientTestCase implements HttpTestRequestListener {
    ...
    @Test
    public void testEscalateToNextSuccessfully() throws Exception {
        OpsGenieClientTestCase.httpServer.setResponseToReturn(new HttpTestResponse("{\"status\":\"successful\", \"took\":1}".getBytes(), 200, "application/json; charset=utf-8"))
        ...
        request.setSource("source1")
        request.setApiKey("customer1")
        request.setEscalationId("escalationId")
        request.setEscalationName("escalationName")

        def response = OpsGenieClientTestCase.opsgenieClient.alert().escalateToNext(request)
        assertTrue(response.isSuccess())
        assertEquals(1, response.getTook())
        ...
        HttpTestRequest requestSent = receivedRequests[0]
        assertEquals(HttpPost.METHOD_NAME, requestSent.getMethod());
        assertEquals("/v1/json/alert/escalateToNext", requestSent.getUrl())
        assertEquals("application/json; charset=utf-8", requestSent.getHeader(HttpHeaders.CONTENT_TYPE));
        ...
        assertEquals("comment", jsonContent[TestConstants.API.NOTE])
        assertEquals("source1", jsonContent[TestConstants.API.SOURCE])
        assertEquals("escalationId", jsonContent[TestConstants.API.ESCALATION_ID])
        assertEquals("escalationName", jsonContent[TestConstants.API.ESCALATION_NAME])
    }

    @Test
    public void testEscalationToNextThrowsExceptionIfRequestCannotBeValidated() throws Exception {
        _testThrowsExceptionIfRequestCannotBeValidated(OpsGenieClientTestCase.opsgenieClient.alert(), "escalateToNext", new EscalateToNextRequest())
    }
