...
import com.ifountain.client.opsgenie.model.alert.AcknowledgeRequest;
...
public class AcknowledgeCommand extends BaseCommand {
    ...
    @Parameter(names = "--" + ClientConstants.API.ALERT_ID, description = "Id of the alert that will be acknowledged. Either this or alias should be given.", splitter = NullSplitter.class)
    private String alertId;

    @Parameter(names = "--" + ClientConstants.API.ALIAS, description = "Alias of the alert that will be acknowledged. Either this or alertId should be given.", variableArity = true, splitter = NullSplitter.class)
    ...
    public void doExecute(IClient client) throws Exception {
        ...
        ((IOpsGenieClient)client).alert().acknowledge(request);
    }
}
